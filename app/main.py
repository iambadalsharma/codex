...
def dashboard(customer_id: int, db: Session = Depends(get_db)):
    ...
    filed_tenders = [t for t in tenders if t.applied.lower() == "yes"]
    won_tenders = [t for t in tenders if t.current_status.lower() == "won"]
    pending_orders = [o for o in orders if o.order_status.lower() not in {"closed", "delivered"}]
    pipeline_value = sum(t.tender_value or 0 for t in tenders)
    ...
    return {
        ...
        "total_filed_tenders": len(filed_tenders),
        ...
        "average_tender_value": pipeline_value / len(tenders) if tenders else 0,
        "win_rate": (len(won_tenders) / len(filed_tenders) * 100) if filed_tenders else 0,
        "pending_order_value": sum(o.total_order_value or 0 for o in pending_orders),
        "urgent_tenders": sum(1 for t in tenders if (d := _due_days(t.submission_end_date)) is not None and 0 <= d <= 7),
    }
